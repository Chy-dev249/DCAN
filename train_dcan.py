...
def train(config):
    ...
    data_batch_source = data_batch(s_gt, batch_size=data_config['source']['batch_size'], drop_last=False, gt_flag=True, num_class=config['network']['params']['class_num'], num_batch=data_config['source']['batch_size'])
    if config['data']['cls_balance']:
        
        dset_loaders['source'] = torch.utils.data.DataLoader(data_set['source'],
                                                        #  batch_size=data_config['source']['batch_size'],
                                                         shuffle=False, num_workers=0, drop_last=False, batch_sampler=data_batch_source)
    else:
        dset_loaders['source'] = torch.utils.data.DataLoader(data_set['source'],
                                                         batch_size=data_config['source']['batch_size'],
    ...
    t_gt = np.array(data_set['target'].imgs)[:, 1]
    data_batch_target = data_batch(t_gt, batch_size=data_config['target']['batch_size'], drop_last=False, gt_flag=False, num_class=config['network']['params']['class_num'], num_batch=data_config['target']['batch_size'])

    if config['data']['cls_balance']:
        dset_loaders['target'] = torch.utils.data.DataLoader(data_set['target'],
                                                        #  batch_size=data_config['target']['batch_size'],
                                                         shuffle=False, num_workers=0, drop_last=False, batch_sampler=data_batch_target)
    else:
        dset_loaders['target'] = torch.utils.data.DataLoader(data_set['target'],
                                                         batch_size=data_config['target']['batch_size'],
                                                         shuffle=True, num_workers=0, drop_last=True)
